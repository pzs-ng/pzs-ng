...
//---------------------------------------------------------------------------
Ztring File__Base::Inform (stream_t StreamKind, size_t StreamPos)
{
    //Integrity
    ...
    ZtringListList Info;
     ...
     if (StreamKind==Stream_General)
        ...
        Info=Video[StreamPos];
    else if (StreamKind==Stream_Audio)
        Info=Audio[StreamPos];
    ...
    //Gestion $if()
    int Position=Retour.find(_T("$if("));
    while (Position>0)
    {
        ZtringList Elements;
        ...
        while (Elements(Elements_Index).SubString(_T("%"), _T("%")).size()>0)
        {
            ...
            Ztring ReplacedBy=Info(ToReplace);
    ...
    while (Retour.SubString(_T("["), _T("]"))!=_T(""))
    {
        ...
        Ztring RemplacerPar=Info(Crochets.SubString(_T("%"), _T("%")));
        ...
        else
        {
            ...
            Ztring Crochets_RemplacerPar=Info(ATraiter.SubString(_T("%"), _T("%")));
    ...
    while (Retour.find(_T("%"), PosX)!=(size_t)-1)
    {
        ...
        {
            Ztring ARemplacer=Ztring(_T("%")+Retour.SubString(_T("%"), _T("%"), PosX))+_T("%");
            Ztring RemplacerPar=Info(Retour.SubString(_T("%"), _T("%"), PosX));
...
//---------------------------------------------------------------------------
void File__Base::Traiter(Ztring &C)
{
    //$if(%a%,zezeze%a%,rrere)
    int Position=C.find(_T("$if("));
    while (Position>0)
    {
