...
public :
    ...
    int Open (const MediaInfo_String &File_Name);
        ...
        /// @retval 0 File not opened
        /// @retval 1 File opened
    int Open (const unsigned char* Begin, size_t Begin_Size, const unsigned char* End=NULL, size_t End_Size=0, int64u FileSize=0);
    ...
    int64u Open_Buffer_Continue_GoTo_Get ();
        /// Open a stream and collect information about it (technical information and tags)
    ...
    MediaInfo_String Inform (size_t Reserved=0);
    ...
    static MediaInfo_String Option_Static (const MediaInfo_String &Option, const MediaInfo_String &Value=_T(""));
...
private :
    ...
    //File
    MediaInfo_String File_Name;
    void*            File_Handle; //Handle of the opened file
    int64u           File_Size;
    int64u           File_Offset;
    ...
    void SelectFromExtension (const Ztring &Parser); //Select File_* from the parser name
    bool LibraryIsModified(); //Is the library has been modified? (#defines...)
    void CreateDummy (const MediaInfo_String& Value); //Create dummy Information
