...
proc ng_free {nick uhost hand chan arg} {
	...
	foreach line [split [exec $binary(DF) "-kP"] "\n"] {
		foreach {name value} [array get tmpdev] {
			if {[string equal [lindex $line 0] [lindex $value 0]]} {
				if {[llength $line] < 4} {
					putlog "dZSbot warning: Invalid \"df -kP\" line: $line"
					continue
				}
	...
	if {[llength [array names tmpdev]]} {
		...
		putlog "dZSbot warning: The following devices had no matching \"df -kP\" entry: [join $devList {, }]"
	}
