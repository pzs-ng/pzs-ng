...
void get_stats(struct VARS *raceI, struct USERINFO **userI) {
	...
	if ( S_ISDIR(fileinfo.st_mode) == 0 ) {
		...
		case '\n':
		    *p_buf = 0;
		    if ( !memcmp(arg[0], "DAYUP", 5)) user[n]->dayup_bytes = atoi(arg[raceI->section * 3 + 2]);
		    else if ( !memcmp(arg[0], "WKUP", 4)) user[n]->wkup_bytes = atoi(arg[raceI->section * 3 + 2]);
		    else if ( !memcmp(arg[0], "MONTHUP", 7)) user[n]->monthup_bytes = atoi(arg[raceI->section * 3 + 2]);
		    else if ( !memcmp(arg[0], "ALLUP", 5)) user[n]->allup_bytes = atoi(arg[raceI->section * 3 + 2]);
		    args = 0;
		    space = 1;
		    break;
		case '\t':
