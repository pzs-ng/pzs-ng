...
main()
{
	...
	if ((raceI.file.name = findfileext(".sfv")) != NULL) {
		...
		while (n--) {
			...
			ext += m;
			/* printf("ext: %s\n",ext); */
			if (!strcomp(ignored_types, ext) && !strcomp(allowed_types, ext) && strcasecmp("sfv", ext) && strcasecmp("nfo", ext) && strcmp(dirlist[n]->d_name + l - 8, "-missing") && strncmp(dirlist[n]->d_name, ".", 1)) {
