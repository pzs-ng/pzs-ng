...
int strplen(char *strin) {
	...
	for (n = 0; isprint(strin[n]) && strin[n]; n++);
...
void showtotals() {
	printf("| Up: %3i / %7.1fkbs | Dn: %3i / %7.1fkbs | Total: %3i / %7.1fkbs |\n",
			uploads, total_up_speed, downloads, total_dn_speed, uploads + downloads,
			total_up_speed + total_dn_speed);
	...
	for ( n = 0 ; n < f_size ; n++ ) {
		if ( f_buf[n] == '\n' || n == f_size ) {
			...
			if ( m != l_start ) {
				...
				g_n_size = m - l_start;
				m        = n;
				for (; f_buf[m] != ':' && m > l_start; m--);
				for (f_buf[m] = 0; f_buf[m] != ':' && m > l_start; m--);
				if ( m != n ) {
					g_id = atoi( f_buf + m + 1 );
