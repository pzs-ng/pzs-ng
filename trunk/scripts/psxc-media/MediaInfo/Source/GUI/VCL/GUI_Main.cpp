...
void __fastcall TMainF::Refresh(TTabSheet *Page)
{
    ...
    unsigned int FilesCount=I->Count_Get();
         ...
         if (Page==Page_Easy)
    {
        ...
        for (int FilePos=0; FilePos<FilesCount; FilePos++)
    ...
    else if (Page==Page_Tree)
    {
        ...
        for (int FilePos=0; FilePos<FilesCount; FilePos++)
        {
            ...
            for (int StreamKind=(int)Stream_General; StreamKind<(int)Stream_Max; StreamKind++)
            {
                ...
                for (int StreamPos=Stream_General; StreamPos<StreamsCount; StreamPos++)
                {
                    //Pour chaque stream
                    ...
                    for (int Champ_Pos=0; Champ_Pos<ChampsCount; Champ_Pos++)
                    {
                        //Pour chaque champ
